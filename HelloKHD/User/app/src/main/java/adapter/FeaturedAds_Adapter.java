...
public class FeaturedAds_Adapter extends Adapter<ViewHolder> {
    ...
    public List<FeaturedAds_FeedItem> feedItems;
    private LayoutInflater inflater;
    public FeaturedAds_Adapter(Activity activity2, List<FeaturedAds_FeedItem> feedItems2) {
        activity = activity2;
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == 0) {
            return new viewHolder(LayoutInflater.from(context).inflate(R.layout.custom_featuredad, parent, false));
        }
        if (viewType == 1) {
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        if (holder instanceof viewHolder) {
            try {
                FeaturedAds_FeedItem item = (FeaturedAds_FeedItem) feedItems.get(position);
                viewHolder viewHolder2 = (viewHolder) holder;
                RequestOptions rep = new RequestOptions().signature(new ObjectKey(item.getImgsig()));
                Glide.with(context).load(Temp.weblink+"advt/"+item.getSn()+".jpg").apply(rep).transition(DrawableTransitionOptions.withCrossFade()).into(viewHolder2.adpic);
                viewHolder2.layout.setOnClickListener(new OnClickListener() {
                    public void onClick(View view) {
                        FeaturedAds_FeedItem item = (FeaturedAds_FeedItem) feedItems.get(position);
                        MainActivity h=(MainActivity)activity;
                        h.openlinks(item.getLinktype(),item.getReference());
                    }
                });
            } catch (Exception e) {
