...
public class DataBase extends SQLiteOpenHelper {
    ...
    public void add_offershows(String title1) {
        drop_offershows();
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(title, title1);
        db.insert(TABLE_name33, null, values);
        db.close();
    }

    public String get_offershows() {
        String link = "";
        Cursor c = getReadableDatabase().rawQuery("SELECT  * FROM "+TABLE_name33, null);
        while (c.moveToNext()) {
            link = c.getString(1);
        }
        c.close();
    ...
    public void drop_offershows() {
        getWritableDatabase().execSQL("delete from "+TABLE_name33);
    }


    public void add_shouldshow(String title1) {
        drop_shouldshow();
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(title, title1);
        db.insert(TABLE_name31, null, values);
        db.close();
    }

    public String get_shouldshow() {
        String link = "";
        Cursor c = getReadableDatabase().rawQuery("SELECT  * FROM "+TABLE_name31, null);
        while (c.moveToNext()) {
            link = c.getString(1);
        }
        c.close();
    ...
    public void drop_shouldshow() {
        getWritableDatabase().execSQL("delete from "+TABLE_name31);
    }

    public void add_showedads(String title1) {
        drop_showedads();
        SQLiteDatabase db = getWritableDatabase();
        ContentValues values = new ContentValues();
        values.put(title, title1);
        db.insert(TABLE_name32, null, values);
        db.close();
    }

    public String get_showedads() {
        String link = "0";
        Cursor c = getReadableDatabase().rawQuery("SELECT  * FROM "+TABLE_name32, null);
        while (c.moveToNext()) {
            link = c.getString(1);
        }
        c.close();
    ...
    public void drop_showedads() {
        getWritableDatabase().execSQL("delete from "+TABLE_name32);
    }
