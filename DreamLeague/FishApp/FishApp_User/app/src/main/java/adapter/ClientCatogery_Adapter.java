...
public class ClientCatogery_Adapter extends Adapter<ViewHolder> {
    ...
    public List<ClientsCatogery_FeedItem> feedItems;
    private LayoutInflater inflater;
    public ClientCatogery_Adapter(Activity activity2, List<ClientsCatogery_FeedItem> feedItems2) {
        activity = activity2;
    ...
    public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
        if (viewType == 0) {
            return new viewHolder(LayoutInflater.from(context).inflate(R.layout.custom_clientcatogery, parent, false));
        }
        if (viewType == 1) {
    ...
    public void onBindViewHolder(ViewHolder holder, final int position) {
        if (holder instanceof viewHolder) {
            try {
                ClientsCatogery_FeedItem item = (ClientsCatogery_FeedItem) feedItems.get(position);
                ...
                Glide.with(context).load(Temp.weblink+"clientcatogery/"+item.getSn()+".png").apply(rep).transition(DrawableTransitionOptions.withCrossFade()).into(viewHolder2.image);
                viewHolder2.layout.setOnClickListener(new OnClickListener() {
                    public void onClick(View view) {
                        ...
                        Intent i = new Intent(context, Client_List.class);
