...
public class FcmMessagingService extends FirebaseMessagingService {
    ...
    //server.appsbag@gmail.com
    public UserDatabaseHandler db;
    public void onMessageReceived(RemoteMessage remoteMessage) {
        db = new UserDatabaseHandler(this);
        if (remoteMessage.getData().size() > 0) {
            try {
                JSONObject jsonObject=new JSONObject(remoteMessage.getData().get("appsbag"));
    ...
    public void generate_newupdate(String msg) {
        ...
        {
            String str = "AppsBagChannel";
            NotificationChannel channel = new NotificationChannel(str, "AppsBagCh", NotificationManager.IMPORTANCE_DEFAULT);
            channel.setDescription("AppsBag_ChNotification");
