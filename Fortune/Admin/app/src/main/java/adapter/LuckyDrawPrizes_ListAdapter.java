...
import com.fortune_admin.Add_LuckyDrawPrize;
...
public class LuckyDrawPrizes_ListAdapter extends BaseAdapter {
    ...
    public LuckyDrawPrizes_ListAdapter(Activity activity2, List<LuckyDrawPrizes_FeedItem> feedItems2) {
        activity = activity2;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            convertView = inflater.inflate(R.layout.custom_luckydrawprizes, null);
        }
        ImageView image=convertView.findViewById(R.id.image);
        TextView name=convertView.findViewById(R.id.name);
        ImageView delete = (ImageView) convertView.findViewById(R.id.delete);
        ImageView edit = (ImageView) convertView.findViewById(R.id.edit);
        LuckyDrawPrizes_FeedItem item = (LuckyDrawPrizes_FeedItem) feedItems.get(position);
        name.setTypeface(face);

        edit.setOnClickListener(new OnClickListener() {
            @Override
            public void onClick(View v) {
                LuckyDrawPrizes_FeedItem item =feedItems.get(position);
                Temp.prizeedit=1;
                Temp.prize_sn=item.getSn();
                Temp.prize_weeks=item.getWeeks();
                Temp.prize_imgsig=item.getImgsig();
                Intent i = new Intent(context, Add_LuckyDrawPrize.class);
        ...
        Glide.with(context).load(Temp.weblink+"prizepics/"+item.getSn()+".jpg").apply(rep).transition(DrawableTransitionOptions.withCrossFade()).into(image);
    ...
    public class delete_data extends AsyncTask<String, Void, String> {
        ...
        public String doInBackground(String... arg0) {
            try {
                String link= Temp.weblink+"delete_drawprizes.php";
        ...
        public void onPostExecute(String result) {
            if (pd != null || pd.isShowing()) {
                ...
                if (result.contains("ok")) {
                    ...
                    ((LuckyDrawPrice) activity).removeitem(pos);
                }
                else{
