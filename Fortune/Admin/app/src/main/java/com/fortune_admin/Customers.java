...
public class Customers extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        text.setText(Temp.ltype_batchname);
        text.setTypeface(face);
        Glide.with(this).load(R.drawable.loading).into(heart);
        feedItems = new ArrayList();
        listAdapter = new Customer_ListAdapter(this, feedItems);
    ...
    public class loadstatus extends AsyncTask<String, Void, String> {
        ...
        public String doInBackground(String... arg0) {
            try {
                String link=Temp.weblink+"getcustomerbybatchid.php";
                String data  = URLEncoder.encode("item", "UTF-8")
                        + "=" + URLEncoder.encode(Temp.ltype_sn+"", "UTF-8");
        ...
        public void onPostExecute(String result) {
            try {
                if (result.contains(":%ok")) {
                    ...
                    int m = -1;
                    for (int i = 1; i <= k; i++) {
                        Customers_FeedItem item = new Customers_FeedItem();
                        m=m+1;
                        item.setSn(got[m]);
                        m=m+1;
                        item.setName(got[m]);
                        m=m+1;
                        item.setHousename(got[m]);
                        m=m+1;
                        item.setPending(got[m]);
                        m=m+1;
