...
public class MainActivity_Old extends AppCompatActivity {
    ...
    public class MainSliderAdapter extends SliderAdapter {
        ...
        public void onBindImageSlide(int position, ImageSlideViewHolder viewHolder) {
            ...
            if (position == 0) {
                ...
                sb.append(str);
                sb.append(MainActivity_Old.imgid);
                sb.append("_1.jpg");
                ...
                sb2.append(str);
                sb2.append(MainActivity_Old.imgid);
                sb2.append("_2.jpg");
                ...
                sb3.append(str);
                sb3.append(MainActivity_Old.imgid);
                sb3.append("_3.jpg");
    ...
    public class loadcatogery extends AsyncTask<String, Void, String> {
        ...
        public void onPreExecute() {
            MainActivity_Old.recylerview1.setVisibility(View.GONE);
            MainActivity_Old.heart1.setVisibility(View.VISIBLE);
        }
            ...
            try {
                if (result.contains(":%ok")) {
                    try {
                        ...
                        int m = -1;
                        MainActivity_Old.db.deleteproductcat();
                        for (int i = 1; i <= k; i++) {
                            int m2 = m + 1;
                            int a = m2;
                            int m3 = m2 + 1;
                            int a1 = m3;
                            m = m3 + 1;
                            MainActivity_Old.db.addproductcatlist(got[a].trim(), got[a1], got[m]);
                        }
                        MainActivity_Old.loadcatogertlist();
                    } catch (Exception e) {
                    }
                } else {
                    MainActivity_Old.heart1.setVisibility(View.GONE);
                }
            } catch (Exception e2) {
            }
        }
    ...
    public class loadstatus extends AsyncTask<String, Void, String> {
        ...
        public void onPreExecute() {
            MainActivity_Old.recylerview.setVisibility(View.GONE);
            MainActivity_Old.heart.setVisibility(View.VISIBLE);
            MainActivity_Old.limit = 0;
        }
        public String doInBackground(String... arg0) {
            String str = "UTF-8";
            try {
                StringBuilder sb = new StringBuilder();
                sb.append(Temp.weblink);
                sb.append("getproducts_home.php");
                String link = sb.toString();
                StringBuilder sb2 = new StringBuilder();
                sb2.append(URLEncoder.encode("item", str));
                sb2.append("=");
        ...
        public void onPostExecute(String result) {
            try {
                if (result.contains(":%ok")) {
                    try {
                        MainActivity_Old.feedItems.clear();
                        String[] got = result.split(":%");
                        ...
                        for (int i = 1; i <= k; i++) {
                            ...
                            m = m3 + 1;
                            item.setshops(got[m]);
                            MainActivity_Old.feedItems.add(item);
                        }
                    } catch (Exception e) {
                    }
                    MainActivity_Old.heart.setVisibility(View.GONE);
                    MainActivity_Old.recylerview.setVisibility(View.VISIBLE);
                    MainActivity_Old.listAdapter.notifyDataSetChanged();
                    return;
                }
                MainActivity_Old.recylerview.setVisibility(View.GONE);
                MainActivity_Old.heart.setVisibility(View.GONE);
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        notilyt.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                try {
                    MainActivity_Old.ndb.deletecount();
                    MainActivity_Old.startActivity(new Intent(MainActivity_Old.getApplicationContext(), Notifications.class));
                } catch (Exception e) {
                }
            }
        });
        ...
        nointernet.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                if (MainActivity_Old.cd.isConnectingToInternet()) {
                    MainActivity_Old.nointernet.setVisibility(View.GONE);
                    new loadstatus().execute(new String[0]);
                    return;
                }
                MainActivity_Old.nointernet.setVisibility(View.VISIBLE);
            }
        });
        ...
        lytvouchers.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                if (MainActivity_Old.udb.get_userid().equalsIgnoreCase("")) {
                    MainActivity_Old.startActivity(new Intent(MainActivity_Old.getApplicationContext(), Registration.class));
                    return;
                }
                MainActivity_Old.startActivity(new Intent(MainActivity_Old.getApplicationContext(), My_Orders.class));
            }
        });
        lytsearch.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                MainActivity_Old.show_search();
            }
        });
        lytcart.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                MainActivity_Old.startActivity(new Intent(MainActivity_Old.getApplicationContext(), Cart.class));
            }
        });
    }
    ...
    public void photoview(final String imgsigs, String imgids) {
        ...
        Slider.init(new ImageLoadingService() {
            public void loadImage(String url, ImageView imageView) {
                Glide.with(MainActivity_Old.getApplicationContext()).load(url).into(imageView);
            }

            public void loadImage(int resource, ImageView imageView) {
                Glide.with(MainActivity_Old.getApplicationContext()).load(Integer.valueOf(resource)).into(imageView);
            }
            ...
            public void loadImage(String url, int placeHolder, int errorDrawable, ImageView imageView) {
                ...
                Glide.with(MainActivity_Old.getApplicationContext()).load(url).apply(requestOptions).transition(DrawableTransitionOptions.withCrossFade()).into(imageView);
            }
        });
        ...
        search.setOnKeyListener(new OnKeyListener() {
            public boolean onKey(View v, int keyCode, KeyEvent event) {
                ...
                if (search.getText().toString().equalsIgnoreCase("")) {
                    Toasty.info(MainActivity_Old.getApplicationContext(), "ദയവായി തിരയുവാനുള്ള എന്തെങ്കിലും ടൈപ്പ് ചെയ്യൂ..", 0).show();
                } else {
                    Temp.catogeryname = search.getText().toString();
                    Product_List.searchtype = "2";
                    Product_List.searchstring = search.getText().toString();
                    MainActivity_Old.startActivity(new Intent(MainActivity_Old.getApplicationContext(), Product_List.class));
                    dialog1.dismiss();
                }
                return true;
            }
    ...
    public void setlocation() {
        ...
        place = (AutoCompleteTextView) dialog.findViewById(R.id.place);
        f17adapter = new CustomListAdapter(this, R.layout.custom_places, placeList);
        place.setAdapter(f17adapter);
        placeList.clear();
        placeList.add("Salman");
        placeList.add("Anees");
        f17adapter.notifyDataSetChanged();
        runOnUiThread(new Runnable() {
            public void run() {
                StringBuilder jsonResults = new StringBuilder();
                try {
                    StringBuilder sb = new StringBuilder("https://maps.googleapis.com/maps/api/place/autocomplete/json");
                    sb.append("?key=AIzaSyCi4seOQkNEwP35QQevfJKjsDmaWwuX-ac");
                    sb.append("&components=country:in");
                    StringBuilder sb2 = new StringBuilder();
                    sb2.append("&input=");
                    sb2.append(URLEncoder.encode("Ponna", "utf8"));
                    sb.append(sb2.toString());
                    InputStreamReader in = new InputStreamReader(((HttpURLConnection) new URL(sb.toString()).openConnection()).getInputStream());
                    ...
                    Log.w("fff", "AA");
                    Log.w("Json", jsonResults.toString());
                    MainActivity_Old.placeList.clear();
                    try {
                        JSONArray predsJsonArray = new JSONObject(jsonResults.toString()).getJSONArray("predictions");
                        if (0 < predsJsonArray.length()) {
                            JSONObject job = predsJsonArray.getJSONObject(0);
                            Log.w("Vinees", job.getJSONObject("structured_formatting").getString("main_text"));
                            MainActivity_Old.gdataclient.getPlaceById(job.getString("place_id")).addOnCompleteListener(new OnCompleteListener<PlaceBufferResponse>() {
                                public void onComplete(@NonNull Task<PlaceBufferResponse> task) {
                                    Place place = (Place) ((PlaceBufferResponse) task.getResult()).get(0);
                                    StringBuilder sb = new StringBuilder();
                                    sb.append(place.getLatLng().latitude);
                                    sb.append("");
                                    Log.w("Loca", sb.toString());
                                }
                            });
                        }
                    } catch (Exception e) {
                    ...
                    Log.w("muhsn", Log.getStackTraceString(a));
                }
            }
        });
        dialog.show();
