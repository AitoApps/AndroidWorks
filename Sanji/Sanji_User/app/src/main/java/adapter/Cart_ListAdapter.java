                    ...
                    try {
                        edit2.setOnClickListener(new OnClickListener() {
                            public void onClick(View view) {
                                try {
                                    Cart_FeedItem cart_FeedItem = (Cart_FeedItem) Cart_ListAdapter.feedItems.get(i3);
                                    Cart_FeedItem item = (Cart_FeedItem) Cart_ListAdapter.feedItems.get(i3);
                                    Cart_ListAdapter.txtpkey = item.getpkey();
                                    ((Cart) Cart_ListAdapter.activity).cartupdate(item.getitemid(), item.getitemname(), item.getitemprice(), item.getMinqty(), item.getUnittype(), item.getqty());
                                } catch (Exception e) {
                                }
                            }
                        });
                        delete.setOnClickListener(new OnClickListener() {
                            public void onClick(View view) {
                                try {
                                    ...
                                    Cart_FeedItem item = (Cart_FeedItem) Cart_ListAdapter.feedItems.get(i3);
                                    Cart_ListAdapter.txtpkey = item.getpkey();
                                    Cart_ListAdapter.pos = i3;
                                    Cart_ListAdapter.showalert("Are you sure want to delete this item from cart?");
                                } catch (Exception e) {
                                }
                            }
                        });
    ...
    public void showalert(String message) {
        ...
        builder.setMessage(message).setPositiveButton("Yes", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                Cart_ListAdapter.db.deletecart_byid(Cart_ListAdapter.txtpkey);
                Cart_ListAdapter.feedItems.remove(Cart_ListAdapter.pos);
                ((Cart) Cart_ListAdapter.activity).calculate_delicharge();
            }
        });
