...
public class Product_List_ByShopCat_ListAdapter extends Adapter<ViewHolder> {
    ...
    public Product_List_ByShopCat_ListAdapter(Activity activity2, List<Productlist_new_FeedItem> feedItems2) {
        activity = activity2;
        ...
        if (holder instanceof viewHolder) {
            try {
                if (position == feedItems.size() - 1 && feedItems.size() > 40) {
                    ((Productist_ByCatogery_FromShops) activity).loadmore();
                }
                    ...
                    if (!item.getogprice().equalsIgnoreCase("0")) {
                        ...
                        viewHolder2.buy.setOnClickListener(new OnClickListener() {
                            public void onClick(View view) {
                                if (Product_List_ByShopCat_ListAdapter.udb.get_userid().equalsIgnoreCase("")) {
                                    Toasty.info(Product_List_ByShopCat_ListAdapter.context, "ദയവായി താങ്കളുടെ മൊബൈല്‍ നമ്പര്‍ റെജിസ്ട്രര്‍ ചെയ്യുക ", 0).show();
                                    Intent i = new Intent(Product_List_ByShopCat_ListAdapter.context, Registration.class);
                                    i.setFlags(268435456);
                                    Product_List_ByShopCat_ListAdapter.context.startActivity(i);
                                    return;
                                }
                                Productlist_new_FeedItem productlist_new_FeedItem = (Productlist_new_FeedItem) Product_List_ByShopCat_ListAdapter.feedItems.get(position);
                                Productlist_new_FeedItem item = (Productlist_new_FeedItem) Product_List_ByShopCat_ListAdapter.feedItems.get(position);
                                ((Productist_ByCatogery_FromShops) Product_List_ByShopCat_ListAdapter.activity).showaddcart(item.getshopid(), item.getsn(), item.getitemname(), item.getprice(), item.getimgsig1(), item.getMinorder(), item.getUnittype(), item.getshopname(), item.getDelicharge(), item.getDelidisc(), item.getMinordramt(), item.getshopimgsig());
                            }
                        });
                        viewHolder2.image.setOnClickListener(new OnClickListener() {
                            public void onClick(View view) {
                                Productlist_new_FeedItem productlist_new_FeedItem = (Productlist_new_FeedItem) Product_List_ByShopCat_ListAdapter.feedItems.get(position);
                                Productlist_new_FeedItem item = (Productlist_new_FeedItem) Product_List_ByShopCat_ListAdapter.feedItems.get(position);
                                ((Productist_ByCatogery_FromShops) Product_List_ByShopCat_ListAdapter.activity).photoview(item.getimgsig1(), item.getsn());
                            }
                        });
                    }
                }
