...
public class Wallets_Report extends AppCompatActivity {
        ...
        this.get.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                if (Wallets_Report.this.mobilenumber.getText().toString().equalsIgnoreCase("")) {
                    Toasty.info(Wallets_Report.this.getApplicationContext(), "Please enter mobile number", 0).show();
            ...
            client.newCall(builder.url(sb.toString()).post(body).build()).enqueue(new Callback() {
                public void onFailure(Call call, IOException e) {
                    Wallets_Report.this.runOnUiThread(new Runnable() {
                        public void run() {
                            Wallets_Report.this.nodata.setVisibility(0);
                            Wallets_Report.this.loading.setVisibility(8);
                            Wallets_Report.this.recylerview.setVisibility(8);
                        }
                    });
                }
                        ...
                        public void run() {
                            try {
                                ...
                                if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("No")) {
                                    ...
                                    Wallets_Report.this.loading.setVisibility(8);
                                    Wallets_Report.this.recylerview.setVisibility(8);
                                    Toasty.info(Wallets_Report.this.getApplicationContext(), "Please contact admin", 0).show();
                                    ...
                                    Wallets_Report.this.loading.setVisibility(8);
                                    Wallets_Report.this.recylerview.setVisibility(0);
                                    Wallets_Report.this.listAdapter.notifyDataSetChanged();
                                } else {
                                    Wallets_Report.this.nodata.setVisibility(0);
                                    Wallets_Report.this.loading.setVisibility(8);
                                    Wallets_Report.this.recylerview.setVisibility(8);
                                    Toasty.info(Wallets_Report.this.getApplicationContext(), "Please try later", 0).show();
                                }
                            } catch (Exception e) {
                                Wallets_Report.this.nodata.setVisibility(0);
                                Wallets_Report.this.loading.setVisibility(8);
        ...
        update.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                if (wallet.getText().toString().equalsIgnoreCase("")) {
                    Toasty.info(Wallets_Report.this.getApplicationContext(), "Please enter waller amount", 0).show();
                    wallet.requestFocus();
                    ...
                    Toasty.info(Wallets_Report.this.getApplicationContext(), "Please make sure your internet connection is active", 0).show();
                }
            }
        });
        dialog1.show();
                    ...
                    Wallets_Report.this.runOnUiThread(new Runnable() {
                        public void run() {
                            try {
                                ...
                                JSONObject jo = new JSONObject(response.body().string());
                                if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("No")) {
                                    Toasty.info(Wallets_Report.this.getApplicationContext(), "Please check contact number", 0).show();
                                } else if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("Update")) {
                                    Toasty.info(Wallets_Report.this.getApplicationContext(), "Updated", 0).show();
    ...
    public void getsupplierdetails() {
        try {
            ...
            StringBuilder sb = new StringBuilder();
            sb.append(Temp_Variable.baseurl);
            sb.append("appadmin/getsupplier_wallet");
            client.newCall(builder.url(sb.toString()).post(body).build()).enqueue(new Callback() {
                public void onFailure(Call call, IOException e) {
                    Wallets_Report.this.runOnUiThread(new Runnable() {
                        public void run() {
