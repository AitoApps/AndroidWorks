    ...
    private void evictToSize(int size) {
        while (this.currentSize > size) {
            ...
            if (Log.isLoggable(arrayAdapter.getTag(), 2)) {
                ...
                StringBuilder sb = new StringBuilder();
                sb.append("evicted: ");
                sb.append(arrayAdapter.getArrayLength(evicted));
                Log.v(tag, sb.toString());
            }
        }
    }
    ...
    public int getCurrentSize() {
        ...
        for (Class<?> type : this.sortedSizes.keySet()) {
            for (Integer size : ((NavigableMap) this.sortedSizes.get(type)).keySet()) {
                currentSize2 += size.intValue() * ((Integer) ((NavigableMap) this.sortedSizes.get(type)).get(size)).intValue() * getAdapterFromType(type).getElementSizeInBytes();
            }
        }
        return currentSize2;
    }
