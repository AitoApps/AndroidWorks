...
public class RegionsList_ListAdapter extends Adapter<ViewHolder> {
    ...
    public RegionsList_ListAdapter(Activity activity2, List<Regions_FeedItem> feedItems2) {
        ...
        builder.setMessage(message).setPositiveButton("Yes", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                if (RegionsList_ListAdapter.this.cd.isConnectingToInternet()) {
                    ...
                    Toasty.info(RegionsList_ListAdapter.this.context, Temp_Variable.nointernet, 0).show();
                }
            }
        });
                        ...
                        public void run() {
                            try {
                                ...
                                if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("No")) {
                                    Toasty.info(RegionsList_ListAdapter.this.context, "Please contact admin", 0).show();
                                } else if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("Update")) {
                                    Toasty.info(RegionsList_ListAdapter.this.context, "deleted", 0).show();
                                    ((Region_Management) RegionsList_ListAdapter.this.activity).removeitem(RegionsList_ListAdapter.this.pos);
                                } else {
                                    Toasty.info(RegionsList_ListAdapter.this.context, "Please try later", 0).show();
                                }
                            } catch (Exception e) {
                                Toasty.info(RegionsList_ListAdapter.this.context, "Please try later", 0).show();
                            }
