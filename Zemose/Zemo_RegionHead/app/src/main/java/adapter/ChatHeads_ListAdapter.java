...
import com.zemose.regionadmin.Chat_History;
import com.zemose.regionadmin.Chatting;
...
public class ChatHeads_ListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ChatHeads_ListAdapter(AppCompatActivity activity2, List<ChatHeads_FeedItem> feedItems2) {
        ...
        if (holder instanceof viewHolder) {
            try {
                ...
                if (this.db.get_unreadcount(item.getissupplier()).equalsIgnoreCase("0")) {
                    viewHolder2.lycount.setVisibility(View.GONE);
                } else {
                    viewHolder2.lycount.setVisibility(View.VISIBLE);
                    viewHolder2.txtcount.setText(this.db.get_unreadcount(item.getissupplier()));
                }
                if (item.getissupplier().contains("s_")) {
                    viewHolder2.medal.setVisibility(View.VISIBLE);
                } else if (item.getissupplier().contains("c_")) {
                    viewHolder2.medal.setVisibility(View.GONE);
                }
