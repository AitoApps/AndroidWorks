...
public class Transfered_Report extends AppCompatActivity {
    ...
    public void getpending_Payments() {
        try {
            ...
            sb.append("appadmin/gettransferd_report_regionhead");
            client.newCall(builder.url(sb.toString()).post(body).build()).enqueue(new Callback() {
                public void onFailure(Call call, IOException e) {
                    Transfered_Report.this.runOnUiThread(new Runnable() {
                        public void run() {
                            Transfered_Report.this.nodata.setVisibility(View.VISIBLE);
                            Transfered_Report.this.loading.setVisibility(View.GONE);
                            Transfered_Report.this.recylerview.setVisibility(View.GONE);
                        }
                    });
                }
                        ...
                        public void run() {
                            try {
                                ...
                                if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("No")) {
                                    ...
                                    Transfered_Report.this.loading.setVisibility(View.GONE);
                                    Transfered_Report.this.recylerview.setVisibility(View.GONE);
                                    Toasty.info(Transfered_Report.this.getApplicationContext(), "Please contact admin", Toast.LENGTH_SHORT).show();
                                    ...
                                    if (jsonarray.length() > 0) {
                                        for (int i = 0; i < jsonarray.length(); i++) {
                                            ...
                                            h.setorderid(jsonobject.getString("orderId"));
                                        ...
                                        Transfered_Report.this.loading.setVisibility(View.GONE);
                                        Transfered_Report.this.recylerview.setVisibility(View.VISIBLE);
                                    ...
                                    Transfered_Report.this.loading.setVisibility(View.GONE);
                                    Transfered_Report.this.recylerview.setVisibility(View.GONE);
                                    Toasty.info(Transfered_Report.this.getApplicationContext(), "Please try later", Toast.LENGTH_SHORT).show();
                                }
                            } catch (Exception a) {
