...
final class Hpack {
    ...
    static final class Writer {
        ...
        /* access modifiers changed from: 0000 */
        public void writeByteString(ByteString data2) throws IOException {
            if (!this.useCompression || Huffman.get().encodedLength(data2) >= data2.size()) {
                writeInt(data2.size(), Hpack.PREFIX_7_BITS, 0);
                this.out.write(data2);
                return;
            }
            Buffer huffmanBuffer = new Buffer();
            Huffman.get().encode(data2, huffmanBuffer);
