...
public final class DeflaterSink implements Sink {
    ...
    public void write(Buffer source, long byteCount) throws IOException {
        ...
        while (byteCount > 0) {
            ...
            this.deflater.setInput(head.f16data, head.pos, toDeflate);
            deflate(false);
    ...
    private void deflate(boolean syncFlush) throws IOException {
        ...
        while (true) {
            ...
            if (syncFlush) {
                deflated = this.deflater.deflate(s.f16data, s.limit, 8192 - s.limit, 2);
            } else {
                deflated = this.deflater.deflate(s.f16data, s.limit, 8192 - s.limit);
            }
            if (deflated > 0) {
                s.limit += deflated;
