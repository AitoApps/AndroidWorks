...
final class CascadingMenuPopup extends MenuPopup implements MenuPresenter, OnKeyListener, OnDismissListener {
    ...
    private void showMenu(@NonNull MenuBuilder menu) {
        ...
        MenuAdapter adapter2 = new MenuAdapter(menuBuilder, inflater, this.mOverflowOnly, ITEM_LAYOUT);
        if (!isShowing() && this.mForceShowIcon) {
            adapter2.setForceShowIcon(true);
        } else if (isShowing()) {
            adapter2.setForceShowIcon(MenuPopup.shouldPreserveIconSpacing(menu));
        }
        int menuWidth = measureIndividualMenuWidth(adapter2, null, this.mContext, this.mMenuMaxWidth);
