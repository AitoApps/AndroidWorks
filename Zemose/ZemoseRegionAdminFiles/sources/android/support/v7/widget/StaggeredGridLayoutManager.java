...
public class StaggeredGridLayoutManager extends LayoutManager implements ScrollVectorProvider {
    ...
    public void setMeasuredDimension(Rect childrenBounds, int wSpec, int hSpec) {
        ...
        if (this.mOrientation == 1) {
            ...
            int chooseSize = chooseSize(wSpec, childrenBounds.width() + horizontalPadding, getMinimumWidth());
    ...
    private int fill(Recycler recycler, LayoutState layoutState, State state) {
        ...
        while (true) {
            if (layoutState.hasMore(state)) {
                ...
                if (assignSpan) {
                    Span currentSpan3 = lp.mFullSpan ? this.mSpans[r92] : getNextSpan(layoutState2);
                ...
                measureChildWithDecorationsAndMargin(view, lp, r92);
                if (layoutState2.mLayoutDirection == z) {
                    if (lp.mFullSpan) {
                    ...
                    if (assignSpan && lp.mFullSpan) {
                        FullSpanItem fullSpanItem = createFullSpanItemFromEnd(start2);
                    ...
                    if (assignSpan && lp.mFullSpan) {
                        FullSpanItem fullSpanItem2 = createFullSpanItemFromStart(end2);
                ...
                if (!isLayoutRTL() || this.mOrientation != z) {
                    ...
                    if (lp.mFullSpan) {
                        ...
                        otherEnd2 = this.mSecondaryOrientation.getEndAfterPadding() - (((this.mSpanCount - z) - currentSpan.mIndex) * this.mSizePerSpan);
                    }
                    otherEnd = otherEnd2;
