...
class DropDownListView extends ListView {
    ...
    public int lookForSelectablePosition(int position, boolean lookDown) {
        ...
        if (!getAdapter().areAllItemsEnabled()) {
            if (lookDown) {
                ...
                while (position2 < count && !adapter2.isEnabled(position2)) {
                    position2++;
                }
            } else {
                int position3 = Math.min(position, count - 1);
                while (position2 >= 0 && !adapter2.isEnabled(position2)) {
    ...
    public int measureHeightOfChildrenCompat(int widthMeasureSpec, int startPosition, int endPosition, int maxHeight, int disallowPartialChildPosition) {
        ...
        while (i3 < count) {
            ...
            int paddingTop3 = adapter2.getItemViewType(i3);
            if (paddingTop3 != viewType) {
                child = null;
                viewType = paddingTop3;
            }
            int i4 = paddingTop3;
            child = adapter2.getView(i3, child, this);
