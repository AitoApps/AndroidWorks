...
public class AppCompatSpinner extends Spinner implements TintableBackgroundView {
    ...
    private static class DropDownAdapter implements ListAdapter, SpinnerAdapter {
        ...
        public DropDownAdapter(@Nullable SpinnerAdapter adapter2, @Nullable Theme dropDownTheme) {
            this.mAdapter = adapter2;
            if (adapter2 instanceof ListAdapter) {
                this.mListAdapter = (ListAdapter) adapter2;
            }
            if (dropDownTheme == null) {
                return;
            }
            if (VERSION.SDK_INT >= 23 && (adapter2 instanceof ThemedSpinnerAdapter)) {
                ThemedSpinnerAdapter themedAdapter = (ThemedSpinnerAdapter) adapter2;
                if (themedAdapter.getDropDownViewTheme() != dropDownTheme) {
                    themedAdapter.setDropDownViewTheme(dropDownTheme);
                }
            } else if (adapter2 instanceof ThemedSpinnerAdapter) {
                ThemedSpinnerAdapter themedAdapter2 = (ThemedSpinnerAdapter) adapter2;
        ...
        public boolean areAllItemsEnabled() {
            ListAdapter adapter2 = this.mListAdapter;
            if (adapter2 != null) {
                return adapter2.areAllItemsEnabled();
            }
            return true;
        }
        ...
        public boolean isEnabled(int position) {
            ListAdapter adapter2 = this.mListAdapter;
            if (adapter2 != null) {
                return adapter2.isEnabled(position);
            }
            return true;
        }
    ...
    public AppCompatSpinner(Context context, AttributeSet attrs, int defStyleAttr, int mode, Theme popupTheme) {
        ...
        if (entries != null) {
            ArrayAdapter<CharSequence> adapter2 = new ArrayAdapter<>(context, 17367048, entries);
    ...
    public void setAdapter(SpinnerAdapter adapter2) {
        if (!this.mPopupSet) {
            this.mTempAdapter = adapter2;
            return;
        }
        super.setAdapter(adapter2);
        if (this.mPopup != null) {
            ...
            this.mPopup.setAdapter(new DropDownAdapter(adapter2, popupContext.getTheme()));
        }
    }
    ...
    /* access modifiers changed from: 0000 */
    public int compatMeasureContentWidth(SpinnerAdapter adapter2, Drawable background) {
        if (adapter2 == null) {
            return 0;
        }
        int width = 0;
        ...
        int end = Math.min(adapter2.getCount(), start + 15);
        for (int i = Math.max(0, start - (15 - (end - start))); i < end; i++) {
            int positionType = adapter2.getItemViewType(i);
            if (positionType != itemType) {
                itemType = positionType;
                itemView = null;
            }
            itemView = adapter2.getView(i, itemView, this);
