...
public final class InputConnectionCompat {
    ...
    static boolean handlePerformPrivateCommand(@Nullable String action, @NonNull Bundle data2, @NonNull OnCommitContentListener onCommitContentListener) {
        boolean z = false;
        if (!TextUtils.equals(COMMIT_CONTENT_ACTION, action) || data2 == null) {
            return z;
        }
        ResultReceiver resultReceiver = null;
        boolean result = false;
        try {
            resultReceiver = (ResultReceiver) data2.getParcelable(COMMIT_CONTENT_RESULT_RECEIVER);
            Uri contentUri = (Uri) data2.getParcelable(COMMIT_CONTENT_CONTENT_URI_KEY);
            ClipDescription description = (ClipDescription) data2.getParcelable(COMMIT_CONTENT_DESCRIPTION_KEY);
            Uri linkUri = (Uri) data2.getParcelable(COMMIT_CONTENT_LINK_URI_KEY);
            result = onCommitContentListener.onCommitContent(new InputContentInfoCompat(contentUri, description, linkUri), data2.getInt(COMMIT_CONTENT_FLAGS_KEY), (Bundle) data2.getParcelable(COMMIT_CONTENT_OPTS_KEY));
            return result;
        } finally {
    ...
    public static InputConnection createWrapper(@NonNull InputConnection inputConnection, @NonNull EditorInfo editorInfo, @NonNull OnCommitContentListener onCommitContentListener) {
        if (inputConnection == null) {
            ...
            return new InputConnectionWrapper(inputConnection, false) {
                public boolean performPrivateCommand(String action, Bundle data2) {
                    if (InputConnectionCompat.handlePerformPrivateCommand(action, data2, listener2)) {
                        return true;
                    }
                    return super.performPrivateCommand(action, data2);
                }
            };
        }
    }
