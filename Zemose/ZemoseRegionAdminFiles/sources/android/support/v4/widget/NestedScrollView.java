...
public class NestedScrollView extends FrameLayout implements NestedScrollingParent2, NestedScrollingChild2, ScrollingView {
    ...
    public boolean executeKeyEvent(@NonNull KeyEvent event) {
        ...
        if (!canScroll) {
            ...
            View nextFocused = FocusFinder.getInstance().findNextFocus(this, currentFocused, BuildConfig.VERSION_CODE);
            if (!(nextFocused == null || nextFocused == this || !nextFocused.requestFocus(BuildConfig.VERSION_CODE))) {
                z = true;
            }
            return z;
        }
        ...
        if (event.getAction() == 0) {
            ...
            if (keyCode != 62) {
                switch (keyCode) {
                    ...
                    case 20:
                        if (event.isAltPressed()) {
                            handled = fullScroll(BuildConfig.VERSION_CODE);
                            break;
                        } else {
                            handled = arrowScroll(BuildConfig.VERSION_CODE);
                            break;
                        }
                }
            } else {
    ...
    public boolean onRequestFocusInDescendants(int direction, Rect previouslyFocusedRect) {
        View nextFocus;
        if (direction == 2) {
            direction = BuildConfig.VERSION_CODE;
        } else if (direction == 1) {
            direction = 33;
        }
