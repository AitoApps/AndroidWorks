        ...
        this.login.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                if (!User_registration.this.cd.isConnectingToInternet()) {
                    Toast.makeText(User_registration.this.getApplicationContext(), Temp_Variable.nointernet, 0).show();
                } else if (User_registration.this.username.getText().toString().equalsIgnoreCase("")) {
                    Toast.makeText(User_registration.this.getApplicationContext(), "Please enter username", 0).show();
                    User_registration.this.username.requestFocus();
                } else if (User_registration.this.password.getText().toString().equalsIgnoreCase("")) {
                    Toast.makeText(User_registration.this.getApplicationContext(), "Please enter password", 0).show();
            ...
            client.newCall(builder.url(sb.toString()).post(body).build()).enqueue(new Callback() {
                public void onFailure(Call call, IOException e) {
                    User_registration.this.runOnUiThread(new Runnable() {
                        public void run() {
                            User_registration.this.pd.dismiss();
                            Toast.makeText(User_registration.this.getApplicationContext(), "Please try later", 0).show();
                        }
                    });
                }
                        ...
                        public void run() {
                            try {
                                ...
                                if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("No")) {
                                    Toasty.info(User_registration.this.getApplicationContext(), "Please contact admin", 0).show();
                                } else if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("nomatch")) {
                                    Toasty.info(User_registration.this.getApplicationContext(), "Please check your username or password", 0).show();
                                } else if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("got")) {
                                    JSONArray jsonarray = jo.getJSONArray("data");
                                    if (jsonarray.length() > 0) {
                                        JSONObject jsonobject = jsonarray.getJSONObject(0);
                                        User_registration.this.udb.addregion(jsonobject.getString("regionheadId"), jsonobject.getString("regionId"), jsonobject.getString("headname"));
                                    ...
                                    Toasty.info(User_registration.this.getApplicationContext(), "Please try later", 0).show();
                                }
                            } catch (Exception e) {
                                User_registration.this.pd.dismiss();
                                Toasty.info(User_registration.this.getApplicationContext(), "Please try later", 0).show();
