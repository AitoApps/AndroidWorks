...
public class Wallet_History extends AppCompatActivity {
    ...
    public void getwallet_history() {
        try {
            ...
            sb.append("appadmin/getwallet_history_regionhead");
            client.newCall(builder.url(sb.toString()).post(body).build()).enqueue(new Callback() {
                public void onFailure(Call call, IOException e) {
                    Wallet_History.this.runOnUiThread(new Runnable() {
                        public void run() {
                            Wallet_History.this.nodata.setVisibility(0);
                            Wallet_History.this.loading.setVisibility(8);
                            Wallet_History.this.recylerview.setVisibility(8);
                        }
                    });
                }
                        ...
                        public void run() {
                            try {
                                ...
                                if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("No")) {
                                    ...
                                    Wallet_History.this.loading.setVisibility(8);
                                    Wallet_History.this.recylerview.setVisibility(8);
                                    Toasty.info(Wallet_History.this.getApplicationContext(), "Please contact admin", 0).show();
                                    ...
                                    Wallet_History.this.loading.setVisibility(8);
                                    Wallet_History.this.recylerview.setVisibility(0);
                                    Wallet_History.this.listAdapter.notifyDataSetChanged();
                                } else {
                                    Wallet_History.this.nodata.setVisibility(0);
                                    Wallet_History.this.loading.setVisibility(8);
                                    Wallet_History.this.recylerview.setVisibility(8);
                                    Toasty.info(Wallet_History.this.getApplicationContext(), "Please try later", 0).show();
                                }
                            } catch (Exception e) {
                                Wallet_History.this.nodata.setVisibility(0);
                                Wallet_History.this.loading.setVisibility(8);
