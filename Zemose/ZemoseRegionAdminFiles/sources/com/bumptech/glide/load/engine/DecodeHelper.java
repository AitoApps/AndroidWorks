...
final class DecodeHelper<Transcode> {
    ...
    public List<Key> getCacheKeys() {
        if (!this.isCacheKeysSet) {
            ...
            int size = loadData2.size();
            for (int i = 0; i < size; i++) {
                LoadData<?> data2 = (LoadData) loadData2.get(i);
                if (!this.cacheKeys.contains(data2.sourceKey)) {
                    this.cacheKeys.add(data2.sourceKey);
                }
                for (int j = 0; j < data2.alternateKeys.size(); j++) {
                    if (!this.cacheKeys.contains(data2.alternateKeys.get(j))) {
                        this.cacheKeys.add(data2.alternateKeys.get(j));
                    }
                }
            }
        }
    ...
    /* access modifiers changed from: 0000 */
    public <X> Encoder<X> getSourceEncoder(X data2) throws NoSourceEncoderAvailableException {
        return this.glideContext.getRegistry().getSourceEncoder(data2);
    }
}
