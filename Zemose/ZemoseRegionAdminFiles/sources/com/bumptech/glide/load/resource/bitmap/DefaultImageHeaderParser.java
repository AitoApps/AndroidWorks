...
public final class DefaultImageHeaderParser implements ImageHeaderParser {
    ...
    private static final class RandomAccessReader {
        ...
        private final ByteBuffer f11data;

        RandomAccessReader(byte[] data2, int length) {
            this.f11data = (ByteBuffer) ByteBuffer.wrap(data2).order(ByteOrder.BIG_ENDIAN).limit(length);
        }

        /* access modifiers changed from: 0000 */
        public void order(ByteOrder byteOrder) {
            this.f11data.order(byteOrder);
        }

        /* access modifiers changed from: 0000 */
        public int length() {
            return this.f11data.remaining();
        }

        /* access modifiers changed from: 0000 */
        public int getInt32(int offset) {
            if (isAvailable(offset, 4)) {
                return this.f11data.getInt(offset);
            }
            return -1;
        }
        ...
        public short getInt16(int offset) {
            if (isAvailable(offset, 2)) {
                return this.f11data.getShort(offset);
            }
            return -1;
        }

        private boolean isAvailable(int offset, int byteSize) {
            return this.f11data.remaining() - offset >= byteSize;
        }
    }
