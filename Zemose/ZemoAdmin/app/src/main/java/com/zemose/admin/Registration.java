        ...
        this.login.setOnClickListener(new OnClickListener() {
            public void onClick(View view) {
                if (Registration.this.skey.getText().toString().equalsIgnoreCase("")) {
                    Toasty.info(Registration.this.getApplicationContext(), "Please enter secret key", Toast.LENGTH_SHORT).show();
                    ...
                    Toasty.info(Registration.this.getApplicationContext(), "Please make sure your internet connection is active", Toast.LENGTH_SHORT).show();
                }
            }
        });
    }
        ...
        FirebaseInstanceId.getInstance().getInstanceId().addOnCompleteListener(new OnCompleteListener<InstanceIdResult>() {
            public void onComplete(@NonNull Task<InstanceIdResult> task) {
                if (!task.isSuccessful()) {
                    ...
                    Toast.makeText(Registration.this.getApplicationContext(), "Please update playservice", Toast.LENGTH_LONG).show();
                    return;
                }
                ...
                client.newCall(builder.url(sb.toString()).post(body).build()).enqueue(new Callback() {
                    public void onFailure(Call call, IOException e) {
                        Registration.this.runOnUiThread(new Runnable() {
                            public void run() {
                                Registration.this.pd.dismiss();
                                Toasty.info(Registration.this.getApplicationContext(), "Please try later", Toast.LENGTH_SHORT).show();
                            }
                        });
                    }
                            ...
                            public void run() {
                                try {
                                    ...
                                    if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("No")) {
                                        Toasty.info(Registration.this.getApplicationContext(), "Please check your secret key", Toast.LENGTH_SHORT).show();
                                    } else if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("Ok")) {
                                        Toasty.info(Registration.this.getApplicationContext(), "Registration Successfull", Toast.LENGTH_SHORT).show();
