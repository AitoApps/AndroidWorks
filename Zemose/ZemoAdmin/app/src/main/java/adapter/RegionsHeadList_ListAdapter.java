...
public class RegionsHeadList_ListAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public RegionsHeadList_ListAdapter(AppCompatActivity activity2, List<RegionsHeads_FeedItem> feedItems2) {
        ...
        builder.setMessage(message).setPositiveButton("Yes", new DialogInterface.OnClickListener() {
            public void onClick(DialogInterface dialog, int id) {
                if (RegionsHeadList_ListAdapter.this.cd.isConnectingToInternet()) {
                    ...
                    Toasty.info(RegionsHeadList_ListAdapter.this.context, Temp_Variable.nointernet, Toast.LENGTH_SHORT).show();
                }
            }
        });
                        ...
                        public void run() {
                            try {
                                ...
                                if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("No")) {
                                    Toasty.info(RegionsHeadList_ListAdapter.this.context, "Please contact admin", Toast.LENGTH_SHORT).show();
                                } else if (jo.getString(PayUmoneyConstants.MESSAGE).equalsIgnoreCase("Update")) {
                                    Toasty.info(RegionsHeadList_ListAdapter.this.context, "deleted", Toast.LENGTH_SHORT).show();
                                    ((Regional_Heads) RegionsHeadList_ListAdapter.this.activity).removeitem(RegionsHeadList_ListAdapter.this.pos);
                                } else {
                                    Toasty.info(RegionsHeadList_ListAdapter.this.context, "Please try later", Toast.LENGTH_SHORT).show();
                                }
                            } catch (Exception e) {
                                Toasty.info(RegionsHeadList_ListAdapter.this.context, "Please try later", Toast.LENGTH_SHORT).show();
                            }
